...
Calnet.prototype.constructor = Calnet;

Calnet.prototype.login = function(callback) {
  ...
  request.post(url, {
    ...
    if (!error && res.statusCode == 200) {
      ...
      return callback(new Error('Error logging in: ' + error));
    }
  });
};

Calnet.prototype.logout = function(callback) {
  var url = 'https://auth.berkeley.edu/cas/logout';
  request.get(url, function (error, res, body) {
    if (!error && res.statusCode == 200) {
      return callback(null, body);
    } else {
      return callback(new Error('Error logging out: ' + error));
    }
  });
};

Calnet.prototype.get = function(url, callback) {
  if (!url) return callback(new Error('No url specified.'));
  request({
    url: url,
    method: 'GET',
    ...
    if (!error && res.statusCode == 200) {
      ...
      return callback(new Error('Error requesting "' + url + '": ' + error));
    }
  });
};

module.exports = Calnet;
